...
class NHentai(IMangaRepository):
    ...
    def __make_request(
        ...
        if self.__config is None:
            raise NhentaiSourceWithoutConfig(
                "Please provide a valid cloudflare cookie and user-agent."
            )
    ...
    def __make_page_uri(
        ...
        if type == "cover":
            return urljoin(self.__TINY_IMAGE_BASE_URL, f"{media_id}/cover.{mime.value}")
        if type == "thumbnail":
            return urljoin(self.__TINY_IMAGE_BASE_URL, f"{media_id}/thumb.{mime.value}")
    ...
    def get(self, identifier: str) -> Manga | None:
        ...
        manga = Manga(
            ...
            authors=[
                Genre(
                    id=genre.get("id"),
                    url=self.__BASE_URL + genre.get("url")[1:],
                    name=genre.get("name"),
                )
                for genre in doujin.get("tags")
                if genre.get("type") == "artist"
            ],
            genres=[
                Genre(
                    id=genre.get("id"),
    ...
    def search(self, query: str, page: int, sort: Sort = Sort.RECENT) -> SearchResult:
        request_response = self.__make_request(
            url=urljoin(self.__BASE_URL, "search"),
            params={
                "q": query,
        ...
        a_tags_with_doujin_id = [
            gallery.find("a", {"class": "cover"}) for gallery in search_results
        ]
    ...
    def paginate(self, page: int) -> Pagination:
        response = self.__make_request(
            url=urljoin(self.__API_URL, f"galleries/all"), params={"page": page}
        )
                ...
                Thumb(
                    ...
                    cover=Image(
                        uri=self.__make_page_uri(
                            ...
                            mime=MIME[
                                result.get("images").get("cover").get("t").upper()
                            ],
                        ),
                        mime=MIME[
                            result.get("images").get("thumbnail").get("t").upper()
                        ],
